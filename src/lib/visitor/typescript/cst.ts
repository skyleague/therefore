...
const defaultZodOutput = (node: Node) => {
    const output = {
        ...
        enabled: (node) =>
            node._validator.type === 'zod' || node._validator.type === 'zod/v3' || node._validator.type === 'zod/v4',
        isTypeOnly: false,
...
export function defaultTypescriptOutput(node: Node): DefinedTypescriptOutput[] {
    ...
    if (node._output === undefined) {
        ...
        if (node._validator.type === 'zod' || node._validator.type === 'zod/v3' || node._validator.type === 'zod/v4') {
