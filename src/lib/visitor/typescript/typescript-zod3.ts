...
export interface TypescriptZodV3WalkerContext {
    ...
    render: (node: Node, ctx?: Partial<TypescriptZodV3WalkerContext>, options?: { allowToZod: boolean }) => string
...
export function buildTypescriptZodV3Context({
    symbol,
...
}): TypescriptZodV3WalkerContext {
    const context: TypescriptZodV3WalkerContext = {
        symbol,
        references,
        locals,
        ...
        render: (
            node,
            ctx: Partial<TypescriptZodV3WalkerContext> = {},
            ...
            return walkTherefore(node, typescriptZodV3Visitor, { ...context, ...ctx })
        },
        declare: (declType: string, node) => asZodV3Declaration(declType, node, { exportSymbol, references }),
...
export function buildTypescriptZodV3TypeContext({
    symbol,
    exportSymbol,
    references,
    locals,
    ...
    context.reference = (node) => {
        if (
            (node._attributes.validatorType?._options.type === 'zod' ||
...
export const typescriptZodV3Visitor: ThereforeVisitor<string, TypescriptZodV3WalkerContext> = {
...
export function asZodV3Declaration(
    declType: string,
    obj: Node,
